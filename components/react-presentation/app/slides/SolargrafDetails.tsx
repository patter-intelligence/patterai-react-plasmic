...
const SolargrafDetails: React.FC<SolargrafDetailsProps> = observer(
  ({ onLoadingChange }) => {
    const [activeTab, _setActiveTab] = useState("DESIGN");
    ...
    useEffect(() => {
      ...
      if (currentStep !== undefined && steps) {
        _setActiveTab(steps[currentStep]);
      }
    }, [currentStep]);

    const setActiveTab = useCallback((tabName: React.SetStateAction<string>) => {
      const steps = appState.currentSlide.get().steps?.map((step) => step.name);
      appState.currentStepIndex.set(steps.indexOf(tabName));
    ...
    return (
      <div className="sd-solargraf-details">
        <div className="sd-tabs">
          ...
          <div
            className={`sd-tab ${
              activeTab === "EFFICIENCY" ? "sd-active" : ""
            }`}
