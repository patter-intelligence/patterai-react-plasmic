...
const QuoteVersions: React.FC = observer(() => {
  ...
  return (
    <div className="qs-quote-versions-container">
      <div className="qs-quote-versions-header">
        <div className="qs-generate-quote-button">
          <button onClick={handleGenerateQuote} disabled={loading}>
            Generate Quote
          </button>
        </div>
      </div>
      <div className="qs-quotes-scroll-container">
        {loading ? (
          <div className="qs-loading-container">
            <div className="qs-loading-spinner"></div>
            <p>Loading quote versions...</p>
          </div>
        ) : error ? (
          <div className="qs-error-container">
            <p>Error: {error}</p>
          </div>
        ) : quotes.length === 0 ? (
          <div className="qs-no-quotes-container">
            <p>No quote versions available. Generate a new quote to get started.</p>
          </div>
        ) : (
          <div className="qs-quotes-grid" ref={quoteGridRef}>
            {quotes.map((quote) => (
              <div
                key={quote.Id}
                className={`qs-quote-box ${quote.isActive__c ? 'active' : ''} ${activatingQuotes[quote.Id] ? 'activating' : ''} ${quote.Id === 'generating' ? 'generating' : ''}`}
                onClick={() => {
              ...
              {activatingQuotes[quote.Id] && (
                <div className="qs-quote-activating-overlay">
                  <div className="qs-loading-spinner"></div>
                  <p>Activating...</p>
                </div>
              )}
...
const GeneratingQuoteOverlay: React.FC = () => (
  <div className="qs-quote-generating-overlay">
    <div className="qs-loading-spinner"></div>
